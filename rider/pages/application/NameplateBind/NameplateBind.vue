<template>
	<view class="container">
		...
		<view class="foot">
			...
			<u-button size="default"  class="pass" type="primary" :disabled="disable" @click.stop="sumbitData()">提交</u-button>
		</view>
			...
			stepFunc(data) {
				...
				if (this.step == 0) {
					if (data === null || data === undefined ) {
						...
						// 如果 productList 不为空，合并数据
						uni.showToast({
							title: data.code + "已绑定" + data.bindCustomerNameplate + '铭牌',
							icon: 'success',
							duration: 2000,
							mask: true,
						});
					...
					if (data === null || data === undefined) {
						...
						this.disable = false
					} else if (this.productList.code && this.productList.bindCustomerNameplate) {
						// 如果 productList 不为空，合并数据
						uni.showToast({
							title: this.productList.code + "已绑定" + this.productList.bindCustomerNameplate + '铭牌',
							icon: 'success',
							duration: 2000,
							mask: true,
						});
