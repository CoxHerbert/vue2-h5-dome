      ...
      <div class="data-content">
        ...
        <div class="table-container">
          <el-table
            ...
            @select-all="selection => handleTableAction('select-all', selection)"
...
/**
 * 自定义选择弹窗组件
 * @param {Object} model - 弹窗配置参数
 * @param {String} model.title - 弹窗标题
 * @param {String} model.defaultLabel - 默认显示的标签键名
 * @param {String} model.defaultValue - 默认选中的值
 * @param {String} model.rowKey - 行数据的唯一键名
 * @param {String} model.showKey - 显示在标签中的键名
 * @param {String} model.dialogGet - 获取弹窗数据的方法
 * @param {String} model.callBack - 处理弹窗数据的回调方法
 * @param {Object} model.defaultParams - 默认参数
 * @param {Object} model.search - 搜索配置
 * @param {Object} model.search.params - 搜索参数
 * @param {String} model.search.placeholder - 搜索占位符
 * @param {String} model.search.width - 搜索宽度
 * @param {Boolean} model.search.clearable - 是否可清除搜索
 * @param {Boolean} model.search.disabled - 是否禁用搜索
 * @param {String} model.search.title - 搜索标题
 * 在原有wf-select-dialog基础上新增了表格全选数据功能 !!!!!!!
 */
...
export default {
  ...
  methods: {
    ...
    closeDialog() {
      ...
      this.tableData = [];
      this.selected = []; // 清空选中状态
      if (this.$refs.tableRef) {
        this.$refs.tableRef.clearSelection(); // 清除表格选中
      }
      this.open = false;
    },
    ...
    /** 表格点击/双击/选择 */
    handleTableAction(action, rowOrSelection) {
      const k = this.masterKey || this.rowKey || 'id';

      // 处理表头全选事件
      if (action === 'select-all' && this.multiple) {
        const selection = rowOrSelection;
        if (selection.length === this.tableData.length) {
          // 全选：将当前页所有数据添加到已选列表（去重）
          const selectedKeys = new Set(this.selected.map(item => item?.[k]));
          ...
          // 取消全选：移除当前页所有已选中的数据
          const tableKeys = new Set(this.tableData.map(row => row?.[k]));
          this.selected = this.selected.filter(item => !tableKeys.has(item?.[k]));
        }
        return;
      }

      // 原有逻辑
      const row = rowOrSelection;
      ...
      if (action === 'row-click' || action === 'select') {
        if (this.multiple) {
          if (has) {
            ...
            this.selected.push({ ...row }); // 创建副本避免引用问题
            this.$refs.tableRef?.toggleRowSelection(row, true);
          }
        } else {
          if (has) {
