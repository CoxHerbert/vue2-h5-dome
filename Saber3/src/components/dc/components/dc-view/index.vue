...
const { iptTagData, currentObject, rowKey } = toRefs(componentData);
watch(
  () => props.modelValue,
  async (newVal, oldVal) => {
    try {
      currentObject.value = cacheData[props.objectName];
      if (!!currentObject.value?.rowKey) {
        rowKey.value = currentObject.value.rowKey;
      }
      let ids;
      ...
      if (rowKey.value !== 'id') {
        ...
        const currentGlobalDataList = Object.keys(currentGlobalData).map(
          key => currentGlobalData[key]
        );
        componentData.iptTagData = ids.map(id => {
          const findItem = currentGlobalDataList.find(item => item[rowKey.value] === id);
