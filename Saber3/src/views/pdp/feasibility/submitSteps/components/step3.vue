<template>
  <div class="wrap-left-form form-step-3">
    <el-form
      class="custom-form"
      ref="formRef"
      :model="formData"
    ...
    >
      ...
      <div class="group-box">
        ...
        <el-form-item class="form-itme-width_50" label="完成时间">
          <div>{{ info.endTime || '-' }}</div>
        </el-form-item>
        <el-form-item class="form-itme-width_50" label="结论">
          <dc-dict type="text" :options="DC_FEA_FEASIBILITY" :value="formData.feaFianlResult" />
        </el-form-item>
      ...
      <div class="group-box" id="bd-review" v-if="info.bdApproved">
        ...
        <el-form-item class="form-itme-width_100" label="是否通过">
          <div
            class="form-item-btn"
            :class="{
              'form-item-btn_active': info.passValue === '通过',
            }"
          >
            通过
          </div>
          <div
            class="form-item-btn"
            :class="{
              'form-item-btn_active': info.passValue === '不通过',
            }"
          >
            不通过
          </div>
...
const pageData = reactive({
  formData: {
    id: null, // 主键id
    feaFianlResult: null, // 是否通过 字典-DC_FEA_FEASIBILITY
    feaFianlConclusion: null, // 最终原因
    feaEvaluationItemIds: null, // 可行性评估项
  },
  rules: {
    feaFianlResult: [
      {
...
const { formData, rules } = toRefs(pageData);

watch(
  () => props.info,
  newInfo => {
    if (newInfo.feaFianlConclusion) {
      formData.value.feaFianlConclusion = newInfo.feaFianlConclusion;
    }
    if (newInfo.feaFianlResult) {
      formData.value.feaFianlResult = newInfo.feaFianlResult;
    }
  },
  { deep: true }
);
