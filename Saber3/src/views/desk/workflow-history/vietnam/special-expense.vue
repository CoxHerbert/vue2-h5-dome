<template>
  <basic-container>
    <div class="content-warp page-special-expense">
      <div class="header">
        <dc-search
          v-if="searchConfig"
          v-model="queryParams"
...
// import AddOrEdit from './page-special-expense/addOrEdit.vue';
...
const getData = async () => {
  ...
  const res = await Api.desk.specialExpense.list(dataparam);
...
const doAction = async (action, scoped = {}) => {
  if (action === 'view') {
    ...
    const params = {
      processKey: 'specialExpenseApplicationProcess',
    };
