...
import AddOrEdit from './page-exchange-rate-list/addOrEdit.vue';
...
const getData = async () => {
  ...
  const res = await Api.scm.exchangeRate.list(dataparam);
...
const handleRowChange = (val, col, item) => {
  ...
  Api.scm.exchangeRate.submit(dataSet).then(res => {
    const { code } = res.data;
    if (code === 200) {
      getData();
    }
...
const deleteIds = ids => {
  proxy
    ...
    .then(() => {
      return Api.scm.exchangeRate.remove({
        ids,
      });
    })
    .then(() => {
