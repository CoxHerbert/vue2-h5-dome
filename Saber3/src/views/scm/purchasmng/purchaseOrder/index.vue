...
const { DC_PURCHASE_REQUEST_DICT, DC_LIST_PROCESS_STATUS, SCMORG_LIST_CACHE } = dictCache;
  ...
  switch (action) {
    ...
    case 'delete':
      proxy
        ...
        .then(() => {
          return Api.scm.purchaseOrder.remove(ids);
        })
        .then(() => {
          proxy.$message({
            type: 'success',
...
const getData = async () => {
  ...
  const res = await Api.scm.purchaseOrder.getList(dataparam);
