<template>
  <div class="addorEdite">
    <div class="title">
      ...
      <div class="title-btns" v-if="title != '查看' || formData.processStatus == '审批中'">
        <el-button
    ...
    >
      ...
      <el-row :gutter="24">
        ...
        <el-col :span="8">
          <el-form-item class="form-itme-width" label="申请日期" prop="applicationDate">
            <el-date-picker
              style="width: 100%"
        ...
        <el-col :span="8">
          <el-form-item class="form-itme-width" label="申请人" prop="applicationUserId">
            <dc-select-user
              v-model="formData.applicationUserId"
              placeholder="请选择申请人"
              :multiple="false"
              :multiple-limit="0"
              :disabled="isDisabled"
        ...
        <el-col :span="8">
          <el-form-item class="form-itme-width" label="申请部门" prop="dept">
            <dc-select-dialog
      ...
      <div class="form-group-title">采购申请物料明细</div>
      <el-button
        type="primary"
        plain
        ...
        >新增</el-button
      >
      <el-table :data="scmPrMaterialList" style="width: 100%" border>
        <el-table-column label="" width="60" v-if="!isDisabled" align="center">
          <template #default="scope">
            <el-button
              ...
              plain
              size="small"
              @click="deleteRow(scope.$index, scmPrMaterialList)"
              :disabled="isDisabled"
            ></el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="materialId" label="物料名称" width="200" align="center">
          <template #default="scoped">
            <dc-select-dialog
              ...
              :multiple-limit="1"
              :clearable="true"
              @change="e => handleSelectMaterial(e, scoped.$index)"
              v-if="!isDisabled"
            />
            <div v-else>{{ scoped.row.materialName }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="requestDate" label="交货日期" align="center" width="200px">
          <template #default="scope">
            <el-date-picker
              style="width: 100%"
              ...
              v-if="!isDisabled"
            />
            <div v-else>{{ scope.row.requestDate }}</div>
          </template>
        </el-table-column>
      </el-table>
    </el-form>
...
// 数据字典
const { DC_PURCHASE_REQUEST_DICT, SCMORG_LIST_CACHE } = proxy.useCache([
...
const data = reactive({
  ...
  rules: {
    ...
    applicationDate: [{ required: true, message: '采购日期为必选项', trigger: 'change' }],
    applicationUserId: [{ required: true, message: '采购员为必选项', trigger: 'change' }],
    dept: [{ required: true, message: '申请部门为必选项', trigger: 'change' }],
  },
  formData: {
    supplierNotext: '系统自动生成',
    applicationUserId: store.getters.userInfo.user_id,
    dept: store.getters.userInfo.dept_id,
    billtypeDict: '1',
...
const { rules, formData, title, scmPrMaterialList, isDisabled, id, filteredSteps, step } =
  toRefs(data);
...
const reject = () => {
  ElMessageBox.prompt(null, '驳回原因', {
    ...
    let res = await Api.scm.application.toExamine(form);
    const { code } = res.data;
    if (code == 200) {
...
const submitForReview = name => {
  proxy.$confirm(`确定${name}吗`).then(async () => {
    ...
    let res = '';
    if (name == '作废') {
      res = await Api.scm.application.invalid(form);
    }
    const { code } = res.data;
    if (code == 200) {
...
const handleSelectMaterial = (row, index) => {
  scmPrMaterialList.value[index].materialId = row.id;
  scmPrMaterialList.value[index].materialName = row.materialName;
  scmPrMaterialList.value[index].materialNumber = row.materialNumber;
  scmPrMaterialList.value[index].brand = row.brand;
  scmPrMaterialList.value[index].specification = row.specification;
  scmPrMaterialList.value[index].unitDict = row.unitDict;
};
...
const reFormData = data => {
  id.value = data.id;
  scmPrMaterialList.value = data.scmPrMaterialList || [];
...
const submitForm = async formEl => {
  formEl.validate(async valid => {
    if (valid) {
      for (let item of scmPrMaterialList.value) {
        if (!item.materialId) {
          proxy.$message.error('物料名称不能为空【采购明细】');
          return false;
        }
        if (!item.quantity) {
          proxy.$message.error('采购数量不能为空【采购明细】');
          return false;
        }
        if (!item.requestDate) {
          proxy.$message.error('交货日期不能为空【采购明细】');
          return false;
        }
      }
      ...
      let data = {
        ...formData.value,
        scmPrMaterialList: scmPrMaterialList.value,
        processStatus: formData.value.processStatus || '开立',
        currentTask: formData.value.currentTask || '开始',
      };
      const res = await Api.scm.application.submit(data);
...
const doAction = action => {
  switch (action) {
    case '提交审核':
      ...
      auditUserRef.value.openDialog(data, Api.scm.application.toExamine, dialogOptions);
      return;
    case '审核通过':
      proxy
        .$confirm(`确定是否通过审核?`, {
        ...
        .then(() => {
          return Api.scm.application.toExamine({
            ...formData.value,
            isPass: 1,
          });
        })
