  ...
  <div class="addorEdite">
          ...
          <template #icon>
            <div class="step-item" :class="getCalss(index)">
              ...
              <span class="step-title" :class="index == 0 ? 'mr20' : ''"
                ...
                <span class="step-desc" v-if="step == 1 && index == 1"> </span>
    ...
    >
      ...
      <el-row :gutter="24">
        ...
        <el-col :span="8">
          <el-form-item class="form-itme-width" label="日期" prop="date">
            <el-date-picker
              style="width: 100%"
      ...
      <el-row :gutter="24">
        <el-col :span="8">
          <el-form-item class="form-itme-width" label="收PO日期" prop="poDate">
            <el-date-picker
              style="width: 100%"
        ...
        <el-col :span="24">
          <el-form-item class="form-itme-width" label="收PO跟进记录" prop="followRecord">
            <el-input
              v-model="formData.followRecord"
              placeholder="请输入收PO跟进备注"
              type="textarea"
              :disabled="isDisabled"
            ></el-input>
          </el-form-item>
      ...
      <div class="form-group-title">预测单物料明细</div>
      <el-button
        type="primary"
        plain
        ...
        >新增</el-button
      >
      <el-table :data="scmForecastMaterialList" style="width: 100%" border>
        <el-table-column label="" width="60" v-if="!isDisabled">
          <template #default="scope">
            <el-button
              ...
              plain
              size="small"
              @click="deleteRow(scope.$index, scmForecastMaterialList)"
              :disabled="isDisabled"
            ></el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="quantity" label="销售数量" align="center" min-width="110">
          <template #default="scope">
            <el-input-number
              style="width: 100%"
              :precision="2"
        ...
        <el-table-column prop="expectStartDate" label="预计开始日期" align="center" width="170px">
          <template #default="scope">
            <el-date-picker
              style="width: 100%"
              v-model="scope.row.expectStartDate"
              type="date"
              placeholder="预计开始日期"
              format="YYYY/MM/DD"
              value-format="YYYY-MM-DD"
        ...
        <el-table-column prop="expectEndDate" label="预计结束日期" align="center" width="170px">
          <template #default="scope">
            <el-date-picker
              style="width: 100%"
...
const data = reactive({
  ...
  rules: {
    ...
    date: [{ required: true, message: '日期为必选项', trigger: 'change' }],
  },
  formData: {
...
const { rules, formData, title, scmForecastMaterialList, isDisabled, id, filteredSteps, step } =
  toRefs(data);
...
const reject = () => {
  ElMessageBox.prompt(null, '驳回原因', {
    ...
    let res = await Api.scm.forecastSheet.toExamine(form);
    const { code } = res.data;
    if (code == 200) {
...
const submitForReview = name => {
  proxy.$confirm(`确定${name}吗`).then(async () => {
    ...
    let res = '';
    if (name == '作废') {
      res = await Api.scm.forecastSheet.invalid(form);
    }
    const { code } = res.data;
    if (code == 200) {
...
const handleSelectMaterial = (row, index) => {
  scmForecastMaterialList.value[index].materialId = row.id;
  scmForecastMaterialList.value[index].materialName = row.materialName;
  scmForecastMaterialList.value[index].materialNumber = row.materialNumber;
  scmForecastMaterialList.value[index].brand = row.brand;
  scmForecastMaterialList.value[index].specification = row.specification;
  scmForecastMaterialList.value[index].unitDict = row.unitDict;
};
...
const getDetail = async () => {
  ...
  const res = await Api.scm.forecastSheet.getDetail({
    id: id.value,
  });
...
watch(
  scmForecastMaterialList.value,
  (newValue, oldValue) => {
    console.log(newValue);
    scmForecastMaterialList.value.forEach(item => {
...
const reFormData = data => {
  ...
  id.value = data.id;
  formData.value = data;
  scmForecastMaterialList.value = data.scmForecastMaterialList || [];
...
const submitForm = async formEl => {
  formEl.validate(async valid => {
    if (valid) {
      for (let item of scmForecastMaterialList.value) {
        if (!item.materialId) {
          proxy.$message.error('物料名称不能为空【销售物料明细】');
          return false;
        }
        if (!item.quantity) {
          proxy.$message.error('销售数量不能为空【销售物料明细】');
          return false;
        }
        if (!item.expectStartDate) {
          proxy.$message.error('预计开始日期不能为空【销售物料明细】');
          return false;
        }
        if (!item.expectEndDate) {
          proxy.$message.error('预计结束日期不能为空【销售物料明细】');
          return false;
        }
        if (!item.deliveryDate) {
          proxy.$message.error('交货日期不能为空【销售物料明细】');
          return false;
        }
      }
      ...
      let data = {
        ...formData.value,
        scmForecastMaterialList: scmForecastMaterialList.value,
        processStatus: formData.value.processStatus || '开立',
        currentTask: formData.value.currentTask || '开始',
        status: formData.value.status || 1,
      };
      const res = await Api.scm.forecastSheet.submit(data);
...
// 添加行
const addRow = () => {
  scmForecastMaterialList.value.push({
...
const doAction = action => {
  switch (action) {
    case '提交审核':
      ...
      auditUserRef.value.openDialog(data, Api.scm.forecastSheet.toExamine, dialogOptions);
      return;
    case '审核通过':
      proxy
        .$confirm(`确定是否通过审核?`, {
        ...
        .then(() => {
          return Api.scm.forecastSheet.toExamine({
            ...formData.value,
            isPass: 1,
          });
        })
