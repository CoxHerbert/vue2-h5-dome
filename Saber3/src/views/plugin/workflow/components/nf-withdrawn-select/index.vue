...
<script>
import { getDetail } from '@/api/system/labor';
...
export default {
  ...
  async mounted() {
    const { DC_LABOR_POSITION } = await this.useCache([{ key: 'DC_LABOR_POSITION' }]);
    this.jobList = DC_LABOR_POSITION;
    this.init();
  },
  methods: {
    init() {
    ...
    // 返回岗位名称
    returnPositionDict(code) {
      if (!code) return;
      let name = this.jobList.find(item => item.dictKey == code)?.dictValue || '-';
      return name;
    },
    onLoad(page, params = {}) {
      this.loading = true;
      ...
      this.$axios.get(this.voUrl, { params: param }).then(res => {
        ...
        this.data = res.data.data.records.map(record => ({
          ...record,
          jobGradeDict: this.returnPositionDict(record.jobGradeDictCode),
        }));
        this.loading = false;
