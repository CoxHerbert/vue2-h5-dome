...
import options from './utils/item-setting';
import api from '@/api/index';
...
const getData = async () => {
  ...
  const res = await Api.crm.rlistTechnologyConfiuge.list({
    ...dataparam,
  });
...
const deleteData = ids => {
  proxy
    ...
    .then(() => {
      api.crm.rlistTechnologyConfiuge.remove({ ids }).then(res => {
        const { code, data } = res.data;
        if (code === 200) {
          getData();
        } else {
...
const doAction = (action, scoped) => {
  if (['edit', 'detail', 'add'].includes(action)) {
    router.push({
      path: `/crm/setting/rlist-technology-configue-setting/${action}/${
        scoped?.row?.id || 'create'
      }`,
    });
