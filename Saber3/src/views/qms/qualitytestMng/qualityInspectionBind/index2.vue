    ...
    <el-drawer v-model="open" :title="title" size="600px" @close="close">
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm" v-loading="loadingSubmit">提交</el-button>
...
const searchConfig = ref({
  paramType: {
    ...
    selectPersonId: {
      ...
      clear(queryParams) {
        queryParams.value.selectPersonId = null;
        queryParams.value.selectPersonName = null;
      },
      getLabelValue(queryParams) {
        if (queryParams.selectPersonName) {
          return `组织：${queryParams.selectPersonName}`;
        }
        return null;
      },
    },
...
const getData = async () => {
  ...
  const res = await Api.mps.plan.getPlanSplitList(dataparam);
